...
def _iter_skill_dirs(root: Path):
    """Yield skill directories under *root*, descending into symlinked packages.

    ``pathlib.Path.rglob`` does not follow directory symlinks, so skill packages
    exposed through symlinks — the documented layout for source-repo-owned skills
    in a shared library — are invisible to ``skill_manage`` even though
    ``skill_view`` resolves them. ``os.walk(followlinks=True)`` descends into
    them; a realpath guard dedupes aliased packages and breaks stray
