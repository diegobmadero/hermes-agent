...
def _commit_model_switch(
    ...
    if not cli._stage_and_swap_model(result, old_model):
        return
    cli._sync_reasoning_state_from_agent()
    if not picker:
...
class CLIModelSwitchMixin:
    ...
    def _sync_reasoning_state_from_agent(self) -> None:
        ...
        if agent is None:
            return
        import copy
        self.reasoning_config = copy.deepcopy(getattr(agent, "reasoning_config", None))
    ...
    def _on_reasoning_update(self, level: str, parsed_config: dict, persist: bool = False) -> bool:
        ...
        Keeps the CLI's own ``reasoning_config`` in sync so later agent re-inits retain the
        tool-set level, and persists to config.yaml only when the model passed ``persist=true``.
        Returns True when the value was durably saved.
        """
        from cli import save_config_value
        self.reasoning_config = parsed_config
        persisted = bool(persist and save_config_value("agent.reasoning_effort", level))
    ...
    def _on_model_update(self, old_model: str, override: dict, scope: str = "session") -> None:
        ...
        ``agent.switch_model`` already swapped the live agent in place; this mirrors the /model
        handler's CLI-state sync so the status line and any later agent re-init derive the
        switched model. Never persists to config.yaml.
        """
        if scope != "session":
            return
