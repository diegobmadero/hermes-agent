...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Master switch for the agent-callable model_switch tool. Off by default: self-model-switching is
        # cost-affecting, so it needs BOTH this flag (check_fn gates the schema) and the model_switch
        # toolset enabled per platform (`hermes tools` → Model Switch).
        "allow_self_model_switch": False,
        # Optional restriction for model_switch targets. Non-empty list = only these models (raw slug or
        # resolved id, case-insensitive) may be targeted, and allowlisted targets skip the expensive-model
        # cost guard. Empty = any resolvable model EXCEPT ones the cost guard flags as expensive (those
        # always need an allowlist entry).
    ...
    "skills": {
        ...
        # Initial system-prompt catalog policy. ``full`` lists every eligible skill with its description;
        # ``names`` lists every name without descriptions; ``featured`` lists only index_featured entries
        # with descriptions; ``off`` omits the catalog. All modes leave skills_list and skill_view access
        # to installed skills unchanged.
        "index_mode": "full",
        "index_featured": [],
