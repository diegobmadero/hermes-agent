...
def _post_swap_argv_tail(args) -> list[str]:
    ...
    if expected_sha is not None:
        from hermes_cli.subcommands.update import normalize_expected_sha
        tail += ["--expected-sha", normalize_expected_sha(expected_sha)]
    return tail
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    try:
        ...
        if expected_sha is not None:
            ...
            _require_expected_sha(
                git_cmd, expected_sha=expected_sha, ref="HEAD", label="Post-swap HEAD")
...
def _finish_pulled_update(
    ...
    """Post-swap tail; a pinned update never resynchronizes fork main after validation."""
