...
def _require_expected_sha(git_cmd, *, expected_sha: str | None, ref: str, label: str) -> None:
    """Exit 1 when *ref* does not resolve to *expected_sha* (dex-main exact-SHA handoff)."""
    if not expected_sha:
        return
    expected = expected_sha.lower()
    result = _git_run(git_cmd, ["rev-parse", ref])
    got = (result.stdout or "").strip().lower()
    if result.returncode != 0 or got != expected:
        print(f"✗ {label} does not match expected SHA.")
        if got:
            print(f"  Got:      {got}")
...
def _pull_updates(
    ...
    in_place_update=False, _windows_gateway_resume=None, expected_sha: str | None = None):
    ...
    movable tracking ref, never detaches — refuses instead of reconciling on any ff failure,
    and verifies the exact HEAD inside the success region, before syntax acceptance."""
    ...
    # Pin mode targets the immutable expected commit in the same role: a tracking ref moved by
    # another writer after the fetch cannot redirect the update.
    merge_ref = expected_sha if expected_sha else (
        target_ref if target_ref is not None else f"origin/{branch}")
    ...
    try:
        try:
            ...
            if expected_sha:
                if _git_run(git_cmd, ["merge", "--ff-only", merge_ref]).returncode != 0:
                    ...
                    sys.exit(1)
            elif merge_ref != f"origin/{branch}":
        ...
        if sync_upstream and not expected_sha:
            ...
            # Pin mode suppresses the fork-main upstream resync entirely.
        ...
        _verify_head_after_pull(
            git_cmd, branch, pre_sync_sha or pre_pull_sha, in_place_update=in_place_update,
            _windows_gateway_resume=_windows_gateway_resume)
        _rollback_if_pulled_syntax_error(git_cmd, pre_sync_sha or pre_pull_sha, expected_sha=expected_sha)
...
def _prepare_checkout_for_update(
    ...
    switch_branch, target_ref=None, _windows_gateway_resume, expected_sha: str | None = None):
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if expected_sha is not None:
        ...
        # No pin: call exactly as upstream does (tests stub the no-kwarg form).
    ...
    if expected_sha is not None:
        # CLI pin (dex-main exact-SHA handoff): carried across the completion seam the
        # same way upstream's release-channel pin is; an explicit pin takes precedence.
        completion_request["expected_sha"] = expected_sha
    ...
    try:
        ...
        if commit_count == 0:
            _require_expected_sha(
                git_cmd, expected_sha=expected_sha, ref="HEAD", label="Current HEAD",
            )
        ...
        pre_pull_sha = _pull_updates(
            ...
            sync_upstream=is_fork and branch == "main" and not release_sha and not expected_sha,
