    ...
    try:
        ...
        if commit_count == 0:
            if expected_sha is not None:
                ...
                if current_sha is None or current_sha.lower() != expected_sha:
                    print("✗ Current HEAD does not match expected SHA.")
                    if current_sha:
                        print(f"  HEAD:     {current_sha.lower()}")
                    print(f"  Expected: {expected_sha}")
                    sys.exit(1)
        ...
        try:
            ...
            if expected_sha is not None:
                updated_sha = _capture_head_sha(git_cmd, _m().PROJECT_ROOT)
