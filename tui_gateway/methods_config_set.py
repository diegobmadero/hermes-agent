...
def _set_reasoning(rid, params, key, value, session):
    ...
    if session and session.get("agent") is not None:
        ...
        session["agent"]._reasoning_config_is_runtime_override = not (scope == "global" or session is None)
        primary_runtime = getattr(session["agent"], "_primary_runtime", None)
        if isinstance(primary_runtime, dict):
            ...
            primary_runtime["reasoning_config_is_runtime_override"] = (
                session["agent"]._reasoning_config_is_runtime_override
            )
        _persist_live_session_runtime(session)
        _emit_session_info(params.get("session_id", ""), session)
