...
def test_todo_state_and_single_handoff_survive_three_fresh_agent_boundaries(
    tmp_path, monkeypatch
):
    hermes_home = tmp_path / "hermes-home"
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
    db = SessionDB(db_path=tmp_path / "state.db")
    session_id = "todo-three-boundary-lifecycle"
    db.create_session(session_id, source="gateway", model="test/model")
    expected = [
        {"id": "active", "content": "Finish the rollout", "status": "in_progress"},
        {"id": "verify", "content": "Verify the gateway", "status": "pending"},
    ]

    agent = _lifecycle_agent(db, session_id)
    agent._todo_store.write(expected, merge=False)
    agent._checkpoint_todo_state()
    history = _assistant_turns(0, 24)
    prompt_todo_counts = []

    def summarize_without_trusting_stale_todos(*args, **kwargs):
        prompt_messages = kwargs.get("messages") or (args[0] if args else [])
        prompt_text = "\n".join(
            str(message.get("content") or "")
            for message in prompt_messages
            if isinstance(message, dict)
        )
        prompt_todo_counts.append(prompt_text.count(TODO_INJECTION_HEADER))
        return _response(
            _valid_zero_user_summary(
                f"Boundary {len(prompt_todo_counts)} completed"
            )
        )
    ...
    for boundary in range(1, 4):
        with patch(
            "agent.context_compressor.call_llm",
            side_effect=summarize_without_trusting_stale_todos,
        ):
        ...
        projected = db.get_messages_as_conversation(session_id)
        fresh = _lifecycle_agent(db, session_id)
        fresh._todo_store._items = []
        fresh._hydrate_todo_store(projected)
        assert fresh._todo_store.read() == expected

        agent = fresh
        history = [*projected, *_assistant_turns(boundary * 30, 24)]

    assert prompt_todo_counts == [0, 0, 0]
    db.close()
