...
def test_repair_merge_merges_multimodal_content_and_drops_stale_sidecar():
    """Multimodal (list) content on the later turn: dex-main merges losslessly —
    silently dropping the later turn's blocks is the exact data loss the
    compaction continuity work fixed. Because the content genuinely changes,
    the stale ``api_content`` sidecar must be dropped; keeping it would replay
    pre-merge bytes to the provider. Deliberate divergence from upstream's
    no-merge / sidecar-preserved choice: a cache miss costs tokens, a lost
    attachment costs correctness."""
    agent = _bare_agent()
    messages = [
        {"role": "user", "content": "Q1"},
        {"role": "assistant", "content": "clean", "api_content": "wire bytes"},
    ...
    assert messages[1]["content"] == [
        ...
        {"type": "text", "text": "img context"},
    ]
    assert "api_content" not in messages[1]


def test_repair_merge_multimodal_degenerate_keeps_sidecar():
    """Negative control for the multimodal branch: an empty later turn yields
    parts identical to the original list, so nothing was rewritten and the
    sidecar must survive (upstream's cache invariant, kept)."""
    agent = _bare_agent()
    prev_parts = [{"type": "text", "text": "clean"}]
    messages = [
        {"role": "user", "content": "Q1"},
        {"role": "assistant", "content": list(prev_parts), "api_content": "wire bytes"},
        {"role": "assistant", "content": ""},
    ]

    AIAgent._repair_message_sequence(agent, messages)

    assert messages[1]["content"] == prev_parts
    assert messages[1]["api_content"] == "wire bytes"
