...
def _reresolve_fallback_reasoning_config(agent) -> None:
    ...
    A live/session override has higher precedence and must survive automatic fallback just as
    it survives an explicit model switch.
    """
    if getattr(agent, "_reasoning_config_is_runtime_override", False):
        logger.info("Fallback %s: preserving runtime reasoning_config: %s", agent.model, agent.reasoning_config)
        return
    try:
