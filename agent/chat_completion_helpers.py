...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Re-resolve reasoning_config for the fallback model only when the
        # current value came from configuration. A live/session override has
        # higher precedence and must survive automatic fallback just as it
        # survives an explicit model switch.
        if getattr(agent, "_reasoning_config_is_runtime_override", False):
            logger.info(
                "Fallback %s: preserving runtime reasoning_config: %s",
                agent.model,
