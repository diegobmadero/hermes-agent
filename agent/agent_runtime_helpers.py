...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset({
    ...
    "delegate_task", "reasoning_effort", "model_switch",
})
...
def _merge_assistant_into(prev: Dict, msg: Dict) -> None:
    ...
    if isinstance(prev_content, str) and isinstance(new_content, str):
        ...
        # Multimodal: merge as content parts rather than leaving the later turn's
        # blocks stranded. A degenerate merge (empty ``new_content``) yields parts
        # identical to the original list and must not count as a rewrite — the same
        # sidecar invariant as the str branch.
        ...
        merged_parts = [*_as_content_parts(prev_content), *_as_content_parts(new_content)]
        content_rewritten = merged_parts != prev_content
...
def switch_model(
    ...
    # Session/runtime reasoning overrides outrank per-model and global config. Without this guard,
    # switching models immediately clobbers a level set by /reasoning or the reasoning_effort tool for
    # the current session. When no runtime override is active, preserve the existing /model contract and
    # re-read the destination model's configured override (per-model > global; YAML False = disabled).
    if getattr(agent, "_reasoning_config_is_runtime_override", False):
        logger.info(
            "switch_model: preserving runtime reasoning_config for %s: %s", agent.model, agent.reasoning_config
        )
    else:
        try:
