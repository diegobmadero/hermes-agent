...
def _preview_reasoning_effort(args: dict, _max_len: int) -> str | None:
    # Show the target level (+ persistence marker).
    level = _oneline(str(args.get("level", "")).strip().lower())
    return (level + " (persist)" if args.get("persist") else level) or None


def _preview_model_switch(args: dict, _max_len: int) -> str | None:
    # Show the target slug (+ turn-scope marker).
    slug = _oneline(str(args.get("slug", "")).strip())
    if not slug:
        return None
    return slug + " (turn)" if str(args.get("scope", "")).strip().lower() == "turn" else slug
...
_PREVIEW_BUILDERS = {
    ...
    "reasoning_effort": _preview_reasoning_effort, "model_switch": _preview_model_switch,
...
_TOOL_VERBS: dict[str, str] = {
    ...
    "reasoning_effort": "Adjusting reasoning effort", "model_switch": "Switching model",
}
