...
def build_skills_system_prompt(
    ...
    index_mode: str = "full", featured_skills: "frozenset[str] | None" = None,
) -> str:
    ...
    ``index_mode`` is profile-level prompt policy: ``full`` preserves the complete catalog, ``names`` keeps
    every eligible name but drops all descriptions, ``featured`` advertises only ``featured_skills``, and
    ``off`` omits the catalog. Featured/off affect prompt disclosure only; the skills tools retain access
    to every installed, non-disabled skill.
    """
    ...
    normalized_featured = frozenset(
        str(name).strip().casefold() for name in (featured_skills or frozenset()) if str(name).strip()
    )
    ...
    try:
        ...
        return _build_skills_system_prompt_inner(
            ...
            index_mode=normalized_mode, featured_skills=normalized_featured)
    finally:
...
def _render_skills_index(
    ...
    if not skills_by_category:
        return ""
    # Profile-level disclosure policy. ``featured`` changes only what the model sees in the initial
    # prompt; skill discovery/loading remains backed by the complete installed skill roots.
    featured_note = ""
    if index_mode == "featured":
        ...
        skills_by_category = {
            category: [(name, desc) for name, desc in items if name.casefold() in wanted]
            for category, items in skills_by_category.items()
        }
        skills_by_category = {category: items for category, items in skills_by_category.items() if items}
        if sum(len(items) for items in skills_by_category.values()) < total_before:
            featured_note = (
    ...
    # index_mode == "names" demotes EVERY category profile-wide, dropping descriptions to cut fixed prompt.
    if index_mode == "names":
...
def _build_skills_system_prompt_inner(
    ...
    cache_key = (
        ...
        index_mode, tuple(sorted(featured_skills or ())),
    )
    with _SKILLS_PROMPT_CACHE_LOCK:
    ...
    result = _render_skills_index(skills_by_category, category_descriptions, compact_categories, available_tools,
                                  index_mode=index_mode, featured_skills=featured_skills)
    with _SKILLS_PROMPT_CACHE_LOCK:
