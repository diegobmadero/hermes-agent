...
def _apply_agent_section(agent, _agent_cfg):
    ...
    with suppress(Exception):
        skills_cfg = _agent_cfg.get("skills", {})
        if not isinstance(skills_cfg, dict):
            skills_cfg = {}
        agent._skill_nudge_interval = int(skills_cfg.get("creation_nudge_interval", 10))
        _index_mode = str(skills_cfg.get("index_mode", "full") or "full").strip().lower()
        if _index_mode not in {"full", "names", "featured", "off"}:
            _index_mode = "full"
        _featured = skills_cfg.get("index_featured", [])
        ...
        agent._skills_index_featured = frozenset(
            str(name).strip().casefold() for name in _featured if str(name).strip()
        )
...
_CALLBACK_PARAMS = (
    ...
    "read_terminal_callback", "reasoning_update_callback", "model_update_callback",
    "read_preview_callback", "drive_preview_callback",
...
def init_agent(
    ...
    reasoning_update_callback: callable = None, model_update_callback: callable = None,
    ...
    reasoning_config: Dict[str, Any] = None, reasoning_config_is_runtime_override: bool = False,
    ...
    Non-obvious parameters:
      ...
      reasoning_update_callback: ``(level, parsed_config, persist) -> bool`` invoked when the
        reasoning_effort tool changes the level; the platform layer scopes it (gateway session
        override) and/or persists it when persist is True. Returns True when persisted.
    ...
    # True only when the active reasoning_config came from a session/runtime override.
    # Model switches preserve such overrides; otherwise they re-resolve the destination
    # model's configured effort.
    agent._reasoning_config_is_runtime_override = bool(reasoning_config_is_runtime_override)
