...
TOOLSETS = {
    ...
    "reasoning": _ts(
        "Adjust runtime reasoning effort for the active session. Off by default "
        "(deliberately not in _HERMES_CORE_TOOLS — zero schema footprint unless "
        "wanted); enable in `hermes tools` → Reasoning Effort.",
        ["reasoning_effort"],
    ),
    "model_switch": _ts(
        "Let the agent switch its own model for complexity routing. Double opt-in: "
        "off by default (not in _HERMES_CORE_TOOLS — zero schema footprint unless "
        "wanted; enable in `hermes tools` → Model Switch) AND gated on "
        "agent.allow_self_model_switch in config.yaml via check_fn.",
        ["model_switch"],
    ),
