...
class TurnRunner:
    ...
    def _build_fresh_agent(self, turn_route, platform_key, combined_ephemeral, max_iterations,
        ...
        return ctx.AIAgent(
            ...
            reasoning_config_is_runtime_override=runner._session_has_reasoning_override(ctx.session_key),
            reasoning_update_callback=runner._make_reasoning_update_callback(ctx.session_key),
            model_update_callback=runner._make_model_update_callback(ctx.session_key),
    ...
    def _wire_turn_agent_callbacks(self, agent, turn_route, reasoning_config,
        ...
        agent._reasoning_config_is_runtime_override = runner._session_has_reasoning_override(ctx.session_key)
        primary_runtime = getattr(agent, "_primary_runtime", None)
        if isinstance(primary_runtime, dict):
            primary_runtime["reasoning_config"] = dict(reasoning_config) if reasoning_config else None
            primary_runtime["reasoning_config_is_runtime_override"] = agent._reasoning_config_is_runtime_override
        agent.reasoning_update_callback = runner._make_reasoning_update_callback(ctx.session_key)
        agent.model_update_callback = runner._make_model_update_callback(ctx.session_key)
