...
class GatewayConfigLoadersMixin:
    ...
    def _session_has_reasoning_override(self, session_key: Optional[str]) -> bool:
        ...
        return bool(state is not None and state.conversation.reasoning_override is not None)
    ...
    def _make_reasoning_update_callback(self, session_key: str):
        """Session override so reasoning_effort survives the cached-agent per-message reset."""
    ...
    def _make_model_update_callback(self, session_key: str):
        """Record a tool-initiated model switch as the same session override /model writes."""
